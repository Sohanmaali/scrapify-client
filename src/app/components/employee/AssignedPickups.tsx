...
const AssignedPickups = () => {
    ...
    try {
    const response:any =  await new BasicProvider('public/scrap/assign-pickup').getRequest();
    ...
    const assignedPickups = pickups.filter((pickup) => pickup.status !== "Completed");
    return (<>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Assigned Pickups</h2>
            ...
            <div className="space-y-4">
              ...
              >
                  ...
                  <div className="flex items-start space-x-4 w-full sm:w-auto">
                      ...
                      {/* Pickup Details */}
                      <div>
                          <h3 className="text-lg font-bold text-gray-800">{pickup.customer?.name || '-'}</h3>
                          <p className="text-gray-600">Address: {pickup?.address|| '-'}</p>
                          <p className="text-gray-600">Date: {pickup?.available_date }</p>
                  ...
                  <div className="flex flex-wrap justify-start sm:justify-end space-y-2 sm:space-y-0 space-x-0 sm:space-x-2 w-full sm:w-auto">
                      {pickup.status !== "Completed" && (
                          <button
                              className="px-4 py-2 bg-lightColor text-darkColor border border-darkColor rounded-lg hover:bg-darkColor hover:text-relatedWhite transition w-full sm:w-auto"
                          >
                              Mark as In Progress
                          </button>
                      )}
                      <button
                          className="px-4 py-2 bg-darkColor text-relatedWhite rounded-lg hover:bg-mutedColor transition w-full sm:w-auto"
                      >
                          Mark as Completed
                      </button>
                  </div>
