...
import { useCityRegions, useCountryRegions, useStateRegions } from "@/hooks/regionHepler";
...
const InputField: React.FC<InputFieldProps> = ({
  ...
  <div className="relative w-full mt-4">
    <input
      ...
      readOnly={disabled}
      className={`peer w-full mt-4 px-3 pt-3.5 pb-0.2 border-b border-gray-300 placeholder-transparent focus:outline-none focus:ring-0 focus:border-darkColor text-mutedColor ${disabled ? "bg-white text-gray-800 cursor-not-allowed" : ""
        }`}
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response: any = await new BasicProvider("customer/profile").getRequest();
  ...
  const updateProfile = async () => {
    try {
      ...
      const data = createFormData({ ...userProfile, featured_image: userProfile?.featured_image?._id });
      ...
      const response: any = await new BasicProvider("customer/update-profile").postRequest(data);
        ...
        <div className="flex flex-wrap lg:flex-nowrap lg:gap-6">
          ...
          <div className="w-full lg:w-9/12 bg-white rounded-lg shadow-lg p-6 mb-4 lg:mb-0">
            ...
            <div className="  gap-4 mt-4">
              <button className="primary-button" role={'button'} onClick={updateProfile}><span className="text">Update</span></button>
          ...
          <div className="w-full lg:w-3/12 bg-white rounded-lg shadow-lg p-6 flex flex-col items-center mb-4 lg:mb-0 lg:order-last order-first">
            <div className="relative user-profile-image">
              <img
                ...
                src={
                  userProfile?.featured_image instanceof File
                    ...
                    : `${process.env.NEXT_PUBLIC_API_URL}/${userProfile?.featured_image?.filepath}` || '/images/noimage.png'
                }
                alt="Profile"
              />
              ...
              {userProfile?.featured_image && <button
                title="Delete"
                ...
                onClick={(e) => setUserProfile((prevProfile: any) => ({
                  ...prevProfile,
                  featured_image: null
                }))}
              >
