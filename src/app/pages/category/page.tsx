...
import CategoryCardSkeleton from "@/app/components/Skeleton/CategoryCardSkeleton";
...
const CategoryPage = () => {
  const { categoryData, loading, error }: any = useAllCategories();
    ...
    <div className="bg-relatedWhite">
            ...
            <button
              className={`flex items-center  gap-2 px-4  text-lg font-medium ${
                categoryType === "all"
                  ...
                  : "text-mutedColor bg-relatedWhite"
              } rounded-lg shadow-md hover:bg-darkColor hover:text-relatedWhite border border-darkColor focus:outline-none focus:ring-2 focus:ring-green-300`}
            ...
            {categoryData.length > 0 &&
              categoryData.map((category: any, index: number) => (
                <button
                  ...
                  onClick={() =>
                    router.push(`?type=${category?.slug}`, { scroll: false })
                  }
                >
                  <span className="text-2xl py-2">
                    <img
      ...
      {loading ? (
        <div className="bg-relatedWHite h-auto py-5">
          <div className="flex  items-center justify-center h-full gap-8 mt-5 px-8 space-x-2">
        ...
        <div className="bg-relatedWHite h-auto py-5">
          {showingCategory.length > 0 &&
            showingCategory.map((category: any, index: number) => (
              <div
                className="flex flex-col items-center justify-center h-full gap-8 mt-5 px-8"
                key={index}
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-10 lg:grid-cols-6 sm:gap-6  w-full justify-items-center">
                  {category?.children?.length > 0 &&
                    category?.children.map((item: any, subIndex: any) => (
                      <Link href={`/pages/addscrap/${item?._id}`}>
                        <CategoryCard data={item} key={subIndex} />
                      </Link>
                    ))}
                </div>
                <hr />
